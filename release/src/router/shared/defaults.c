...
//	{ "wl_interfmode", "2", 0 },		/* Interference mitigation mode (2/3/4) */
	...
	{ "wl_btc_mode", "0", 0 },		/* bluetooth coexistance(BTC) mode Disable(0), Enable(1), Pre-emptive mode(2) */
#endif
...
#if defined (RTCONFIG_BCMARM) && !defined (RTCONFIG_BCM7) && !defined (RTCONFIG_BCM_7114) && !defined (RTCONFIG_BCM9)
...
#else
	{ "wl_turbo_qam", "0", 0 },
#if !defined(RTCONFIG_BCM9) && !defined(RTAC56U) && !defined(RTAC56S) && !defined(RTAC88U) && !defined(RTAC3100) && !defined(RTAC5300) && !defined(RTAC5300R)
	{ "wl0_turbo_qam", "1", 0 },
#endif
#endif
#endif	// BCMARM
...
#ifdef RTCONFIG_QTN
	{ "wl1_bw", "3", 0 },		/* 0/1/2/3 auto/20/40/80MHz */
#endif
#else
...
// WEP data encryption 0, 1, 2 : disabled/5/13
	...
	{ "wl_mumimo", "1", 0 },
#elif defined(RTCONFIG_QTN) || defined(RTCONFIG_QTN) || defined(RTCONFIG_MUMIMO)
	{ "wl_mumimo", "0", 0 },
#endif
...
//	{ "wl1_HT_RxStream", "3" }, // move to init_nvram for model dep.
	...
	{ "emf_enable",	"0", 0 },		/* it is common entry for all platform now
...
// Wireless Mac Filter
	...
	{ "wl_atf", "0", 0 },			/* disable/enable Airtime Fairness */
	{ "wl_atf_mode", "0", 0 }, 		/* ATF mode: 0/1/2(by client/SSID/client+SSID) */
	{ "wl_atf_sta", "", 0 }, 		/* ATF by client: <MAC>%<MAC>% */
	{ "wl_atf_ssid", "", 0 }, 		/* ATF by SSID: <%<%<%<% */
#endif
	...
	/* Tx Beamforming */
#ifdef RTCONFIG_BCMARM
	{ "wl_txbf_bfr_cap", "1", 0 },		/* 2 for mu-mimo case */
	{ "wl_txbf_bfe_cap", "1", 0 },		/* 2 for mu-mimo case */
#endif
#ifdef RTCONFIG_BCMARM
#ifndef RTCONFIG_WIFILOGO
	{ "wl_txbf_imp", "1", 0 },
	...
	{ "wl_mu_features", "0", 0},		/* mu-features=0x8000 when mu-mimo enabled */
#endif
#else
	{ "wl_txbf_timer", "25", 0 },
#endif
	...
	{"wl1_bsd_sta_select_policy_x", "10 0 0 500 0 1 1 0 0 0 0x60", 0 },
	{"wl2_bsd_sta_select_policy_x", "10 0 900 0 0 1 1 0 0 0 0x68", 0 },
	...
	{"bsd_bounce_detect_x", "180 2 3600", 0 },
	{"bsd_aclist_timeout", "3", 0},
...
//#ifdef RTCONFIG_SWITCH_CONTROL_8367
	...
	{ "wans_ntool_unit", "0" },	/* Allow user select WAN unit for ping/traceroute command in network tool page if dualwan + load-balanced. */
...
// the following variables suppose can be removed
	...
	{ "dslx_link_enable", "1" },
	{ "dslx_DHCPClient", "1" },
	{ "dslx_dhcp_clientid_type", "0" },/* 0: MAC 1: Node-specific RFC4361 */
	{ "dslx_dhcp_clientid", "" },	//Required by some ISP using RFC 1483 MER.
	{ "dslx_dhcp_vendorid", "" },	//DHCP option 60
...
//
	...
	{ "qos_unit", "0" },		/* Last configured QoS */
#endif
	...
	{ "pppoerelay_unit", "0" },	/* Select WAN unit if dualwan + load-balanced. */
#endif
	{ "misc_http_x", "0" },
...
//#endif
	...
	{"chilli_radius", "cpfree.hk"},
#else
	{"chilli_radius", "cpfree.com"},
#endif
	{"chilli_backup", "aroundwifi.com"},
#ifdef RTCONFIG_HK_CPFREE
	{"chilli_url", "http://www.cpfree.hk/loginIndex.action"},	
#else	
	{"chilli_url", "http://www.cpfree.com/loginIndex.action"},
#endif
	{"chilli_pass", "cp27nlaz09s8a"},
	{"chilli_uamsecret", "ua8k69fo92n"},
#else
	{"chilli_enable", "0"},
	...
	{"chilli_dns1", "8.8.8.8"},
	{"chilli_interface", "br1"},
	{"chilli_radiusnasid", "radius"},
	{"chilli_uamanydns", "0"},
	{"chilli_uamallowed", ""},
	{"chilli_macauth", "0"},
	...
	{ "firmware_server", "https://fwupdate.lostrealm.ca/asuswrt-merlin" },
	...
	{ "vpn_client4_ncp_enable",	"1"		},
	{ "vpn_client4_ncp_ciphers",	"AES-128-GCM:AES-256-GCM:AES-128-CBC:AES-256-CBC"},
	{ "vpn_client4_digest",		"default"	},
	...
	{ "vpn_client5_ncp_enable",	"1"		},
	{ "vpn_client5_ncp_ciphers",	"AES-128-GCM:AES-256-GCM:AES-128-CBC:AES-256-CBC"},
	{ "vpn_client5_digest",		"default"	},
	...
	{ "vpn_client_ncp_enable",	"1"		},
	{ "vpn_client_ncp_ciphers",	""		},
	{ "vpn_client_digest",		"default"	},
...
#ifdef RTCONFIG_IPV6
	// IPv6 parameters
	{ "ipv6_service",	"disabled"	},	// disabled/ipv6pt/dhcp6/6in4/6to4/6rd/other
	{ "ipv6_ifdev",		"ppp"		},
	...
	// IPv6 parameters
	{ "ipv61_service",	"disabled"	},	// disabled/dhcp6/ipv6pt/6in4/6to4/6rd/other
	{ "ipv61_ifdev",	"ppp"		},
	{ "ipv61_prefix",	""		},	// The global-scope IPv6 prefix to route/advertise
	{ "ipv61_prefix_length","64"		},	// The bit length of the prefix. Used by dhcp6c. For radvd, /64 is always assumed.
	{ "ipv61_dhcp_pd",	"1"		},	// Enable DHCP-PD (DHCP Prefix Delegation) by default
	{ "ipv61_rtr_addr",	""		},	// defaults to $ipv6_prefix::1
	{ "ipv61_prefix_len_wan","64"		},	// used in ipv6_service other
	{ "ipv61_ipaddr",	""		},	// used in ipv6_service other
	{ "ipv61_gateway",	""		},	// used in ipv6_service other
	{ "ipv61_radvd",	"1"		},	// Enable Router Advertisement (radvd)
	{ "ipv61_relay",	"192.88.99.1"	},	// IPv6 Anycast Address
	{ "ipv61_tun_v4end",	"0.0.0.0"	},	// Foreign IPv4 endpoint of SIT tunnel
	{ "ipv61_tun_addr",	""		},	// IPv6 address to assign to local tunnel endpoint
	{ "ipv61_tun_addrlen",	"64"		},	// CIDR prefix length for tunnel's IPv6 address
	{ "ipv61_tun_peer",	""		},	// IPv6 address of remote tunnel endpoint
	{ "ipv61_tun_mtu",	"0"		},	// Tunnel MTU, 0 for default
	{ "ipv61_tun_ttl",	"255"		},	// Tunnel TTL
	{ "ipv61_6rd_dhcp",	"1"		},
...
#if 0
	{ "ipv61_dns",		""		},	// DNS server(s) IPs
#else
	{ "ipv61_dns1",		""		},
	{ "ipv61_dns2",		""		},
	{ "ipv61_dns3",		""		},
#endif
	{ "ipv61_get_dns",	""		},	// DNS IP address which got by dhcp6c
	{ "ipv61_dnsenable",	"1"		},
	{ "ipv61_get_domain",	""		},	// domain name which got by dhcp6c
	{ "ipv61_autoconf_type","0"		},	// IPv6 LAN auconfiguration type, 0: stateless, 1: stateful
	{ "ipv61_dhcp_lifetime","86400"		},	// valid lifetimes of IPv6 prefix in seconds
	{ "ipv61_dhcp_start",	""		},	// start of address pool
	{ "ipv61_dhcp_end",	""		},	// end of address pool
	...
	{ "captive_portal_5g",			"" },
	{ "captive_portal_5g_2",		"" },
	{ "captive_portal_2g_if",		"off" },
	{ "captive_portal_5g_if",		"off" },
	{ "captive_portal_5g_2_if",		"off" },
	{ "captive_portal_2g_if_temp",		"off" },
	{ "captive_portal_5g_if_temp",		"off" },
	{ "captive_portal_5g_2_if_temp",	"off" },
