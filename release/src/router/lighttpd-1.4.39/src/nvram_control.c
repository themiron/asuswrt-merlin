...
#define AICLOUD_ACC_LIST "aicloud_acc_list"
#define AICLOUD_ACC_INVITE_LIST "aicloud_acc_invite_list"
...
char* nvram_get_aicloud_acc_list(void)
{
	...
	tcapi_get(WEBDAV, AICLOUD_ACC_LIST, aicloud_acc_list);
	return aicloud_acc_list;
#else
	return nvram_get(AICLOUD_ACC_LIST);
#endif
}
...
char* nvram_get_aicloud_acc_invite_list(void)
{
	...
	tcapi_get(WEBDAV, AICLOUD_ACC_INVITE_LIST, aicloud_acc_invite_list);
	return aicloud_acc_invite_list;
#else
	return nvram_get(AICLOUD_ACC_INVITE_LIST);
#endif
}
...
#ifdef USE_TCAPI
	char apps_sq[2] = {0};
	tcapi_get(APPS, APPS_SQ, apps_sq);
	return apps_sq;
#else
	return 0;
#endif
